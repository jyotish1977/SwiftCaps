...
{
    public class QuizAverageReportTests : CRUDTestBase
    {
        [Fact]
        public async Task When_NoQuizzes_Should_GenerateReport()
        {
            using var context = GetDbContext();
                await SeedGroups(context);
                var sut = new AdminReportingService(context);
                var report = await sut.GetQuizAverageReport();
                report.Count.ShouldBe(0);
        }

        [Fact]
        public async Task When_QuizAvailable_NoSchedules_Should_GenerateReport()
        {
            using var context = GetDbContext();
        ...
        [Fact]
        public async Task When_QuizAvailable_ScheduleAssigned_NoUsersInGroup_Should_GenerateReport()
        {
            using var context = GetDbContext();
                await SeedGroups(context);
                await SeedQuizzes(context);
        ...
        public async Task When_ScheduleAssigned_GroupAssigned_UserCompletedQuizzes_Should_GenerateReport()
        {
            using var context = GetDbContext();
                ...
                var sut = new AdminReportingService(context);
                var report = await sut.GetGroupAverageReport();
                report.Count.ShouldBe(2);
                report[0].GroupName.ShouldNotBeNullOrEmpty();
                report[0].QuizCount.ShouldBe(1);
                report[0].AverageDonePercentage.ShouldBe(100);
                report[0].AvergageTime.ShouldBe("9m 0s");
                report[0].UserCount.ShouldBe(5);

                report[0].GroupName.ShouldNotBeNullOrEmpty();
                report[0].QuizCount.ShouldBe(1);
                report[1].AverageDonePercentage.ShouldBe(80);
